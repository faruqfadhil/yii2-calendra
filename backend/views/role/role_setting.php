...
{
    $ch = "<div style='width: 150px;float: left'><div class=''><label><input type=checkbox name=$name value='$value' $check /> $str </label></div></div>";
    return $ch;
}
                ...
                <?php foreach (\app\models\Menu::find()->all() as $no => $menus) { ?>
                    <tr>
                        ...
                        <td>
                            ...
                            foreach (RoleMenu::find()->where(["role" =>$urole])->all() as $role) {
                                array_push($arr_role, $role->menu);

                                foreach (Action::find()->where(["role" =>$urole,'menu'=>$role->menu])->all() as $action) {
